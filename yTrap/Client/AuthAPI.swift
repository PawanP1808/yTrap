...
class AuthAPI {
    ...
    func getAuthCode(authToken token:String, accessToken completion:@escaping (_ accessToken:String,_ refreshToken: String,String)->()) {
        ...
        Alamofire.request(Constants.API.Spotify.tokenAuthSpotifyUrl, method: .post, parameters: parameters, headers:nil).responseJSON { response in
    ...
    func refreshToken(withRefreshToken token: String) -> (String?,String?) {
        ...
        let response = Alamofire.request(Constants.API.Spotify.tokenAuthSpotifyUrl, method: .post, parameters:parameters,headers:nil).responseJSON()
        if response.response?.statusCode == 200 {
